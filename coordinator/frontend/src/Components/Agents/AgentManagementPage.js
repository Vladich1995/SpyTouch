...
const AgentManagementPage = () => {
    ...
    useEffect(() => {
      const sendRequest = async () => {
        try{
            if(location != ""){
              const response = await fetch(`http://localhost:5000/getagents/${location}`);
    ...
    const addAgentHandler = async (newAgent) => {
      ...
      try{
        await fetch("http://localhost:5000/addagent", {
          method: "POST",
          headers: {
    ...
    const  deleteAgent = async () => {
      try{
        await fetch(`http://localhost:5000/deleteagent/${agentToDelete.id}`, {
        method: "DELETE",
        headers: {
          "Content-Type" : "application/json"
        }
