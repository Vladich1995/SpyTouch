...
const Chat = (props) => {
    ...
    if (socket) {
      socket.on('ping', async (message) => {
        ...
        try{
          ...
          const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/getmessages/${message.agentId}`);
  ...
  useEffect(() => {
    const loadMessages = async () => {
      try{
        const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/getmessages/${props.contact.id}`);
        const responseData = await response.json();
        updateMessagesList(responseData.messages);
      } catch (err) {
  ...
  const submitHandler = async (event) => {
    ...
    try{
      await fetch(`${process.env.REACT_APP_BACKEND_URL}/storemessage`, {
      method: "POST",
      headers: {
